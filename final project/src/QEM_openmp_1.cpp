...
void simplify() {
    ...
    vector<vector<ver_pair>> heap_(MAX_THREADS);

    #pragma omp parallel for
    for (u = 0; u < vertex_num; u++) {
        int id = omp_get_thread_num();  // 獲取執行緒編號
        for (auto it = edges[u].begin(); it != edges[u].end(); it++) {
            ...
            // 將結果加入對應執行緒的 heap
            heap_[id].push_back(ver_pair(u, v, err, x, y, z, 1));
        }
    }

    // 將所有執行緒的結果彙整到主 heap 中
	...
	for (int cnt = 0; cnt < target; cnt++) {
		...
		vertices[u].computeQ();
		vector<int> erase_edges;
		for (auto it = edges[u].begin(); it != edges[u].end(); it++) {
			v = *it;
			if (pre[v] != v) {
				erase_edges.push_back(v);
				continue;
			}
			vertices[v].computeQ();
			assert(u != v);
		...
		for(auto v: erase_edges) edges[u].erase(v);
	}
	fprintf(stderr, "%.6lf\n", cost);
}
